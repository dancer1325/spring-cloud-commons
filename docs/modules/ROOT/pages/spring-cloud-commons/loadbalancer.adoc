...
* own client-side load-balancer
  ** abstract
  ** implementation
* `ReactiveLoadBalancer`
  ** has *Round-Robin-based* & *Random* implementations
    *** instances -- can be selected via -- `ServiceInstanceListSupplier`
      **** built-in service-discovery-based implementation of `ServiceInstanceListSupplier`
-- retrieves, via xref:spring-cloud-commons/common-abstractions.adoc#discovery-client[Discovery Client] | classpath  -- available instances | Service Discovery
* `spring.cloud.loadbalancer.enabled`
  ** allows
    *** disable / enable Spring Cloud LoadBalancer

[[eager-loading-of-loadbalancer-contexts]]
== Eager loading of LoadBalancer contexts

* Spring Cloud LoadBalancer creates a separate Spring child context / service id
  ** initialization of these contexts
    *** By default, lazily == once first request / service id -- is being -- load-balanced
    *** `spring.cloud.loadbalancer.eager-load.clients[indexOfClient]=clientId`
      **** load those contexts eagerly

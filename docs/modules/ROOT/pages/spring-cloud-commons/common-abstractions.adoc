...
* uses
  ** 👁️there is NO Service-Registry-backed `DiscoveryClient` | classpath 👁️
* allows
  ** getting -- via properties -- information about service & instances
    *** `spring.cloud.discovery.client.simple.instances.serviceId[indexNumberOfTheInstance].uri=UriToFindTheInstance`
      **** way to indicate the available instances

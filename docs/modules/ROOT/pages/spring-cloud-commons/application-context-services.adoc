...
* Spring Boot
  ** 's view == how to build an application -- via -- Spring
  ** has
    *** common configuration files | conventional locations
    *** endpoints -- for -- common management & monitoring tasks
* Spring Cloud
  ** Spring Boot + features / MANY components | system use

[[the-bootstrap-application-context]]
== The Bootstrap Application Context

* Spring Cloud application
  ** 💡operates -- by creating a -- "`bootstrap`" context 💡
    *** 👀== parent context | main application👀
    *** -- responsible for --
      **** loading external sources' configuration properties -- for decrypting -- properties | local external configuration files
* "`bootstrap`" context & "`main`" context
  ** 👀share an `Environment`👀
    *** == Spring application's source of EXTERNAL properties
* bootstrap properties
  ** ⚠️!= `bootstrap.properties`⚠️
  ** == properties / loaded | bootstrap phase
    *** added / HIGH precedence -> NOT overridden -- by -- local configuration
...
* if you build an application context ( -- from -- `SpringApplication` or `SpringApplicationBuilder`) -> Bootstrap context == application context' parent /
  ** ⭐️("`main`" application context + Spring Cloud Config)'s property sources > ("`main`" application context)'s property sources⭐️
    *** Reason: 🧠child contexts -- inherit -- property & profiles -- from -- their parent🧠
      **** == Spring's feature
    *** ADDITIONAL property sources
      **** "`bootstrap`"
        ***** TODO: If any `PropertySourceLocators` are found in the bootstrap context and if they have non-empty properties, an optional `CompositePropertySource` appears with high priority.
